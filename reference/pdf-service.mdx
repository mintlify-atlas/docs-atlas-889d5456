---
title: "PdfService y PresupuestoPdfService: generación de PDFs en LEFA"
...
| Worker | Servicio que invoca | Señal de finalización |
|---|---|---|
| `pdf_worker.py` | `PDFService.generar` | `pdf_generado` (emite la `Path` del archivo) |
| `presupuesto_pdf_worker.py` | `PresupuestoPDFService.generar` | `pdf_generado` (emite la `Path` del archivo) |

Los workers heredan de `QThread` y emiten señales Qt para notificar a la ventana principal cuando el PDF está listo o cuando se produce un error, permitiendo abrir el visor del sistema operativo de forma automática tras la generación.
