---
title: "Constantes y rutas de configuración de LEFA"
sidebarTitle: "Constantes"
description: "Referencia de las constantes, rutas de datos y valores por defecto definidos en config.py de LEFA: directorios, tipos de IVA, series y keyring."
---
...
`ensure_directories()` se invoca automáticamente al arrancar LEFA (antes de mostrar la interfaz) y garantiza que todas las carpetas de datos existan. Es idempotente: si los directorios ya existen, no hace nada.
...
Los directorios creados son `~/.lefa`, `~/.lefa/facturas_pdf`, `~/.lefa/presupuestos_pdf`, `~/.lefa/verifactu/registros` y `~/.lefa/facturas_xml`. La carpeta `~/.lefa/smtp_config.json` y `~/.lefa/preferencias.json` son archivos, no directorios, por lo que su carpeta padre (`APP_DATA_DIR`) ya queda cubierta por la primera llamada.
...
El diccionario `FORMATOS_NUMERACION` asocia cada identificador con una plantilla de marcadores (`{serie}`, `{anio}`, `{seq}`):

| `formato_id` | Plantilla | Ejemplo con serie `FACT`, año `2026`, seq `1` |
|---|---|---|
| `fact_anio_4` | `{serie}-{anio}-{seq}` | `FACT-2026-0001` |
...
| `serie_seq3` | `{serie}-{seq}` | `WEB-001` |

El número de dígitos del campo `{seq}` se controla con la preferencia `digitos_secuencia` (por defecto `4`). El valor por defecto de `formato_numeracion` es `"fact_anio_4"`.
